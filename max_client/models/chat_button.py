...
class ChatButton(Button):
    ...
    def __init__(
        ...
        """ChatButton - a model defined in OpenAPI."""
    ...
    def chat_title(self, chat_title):
        ...
        if chat_title is None:
            raise ValueError("Invalid value for `chat_title`, must not be `None`")
        if chat_title is not None and len(chat_title) > self.CHAT_TITLE_MAX_LENGTH:
            raise ValueError(
                f"Invalid value for `chat_title`, length must be less than or equal to `{self.CHAT_TITLE_MAX_LENGTH}`"
            )
    ...
    def chat_description(self, chat_description):
        ...
        if (
            ...
            raise ValueError(
                f"Invalid value for `chat_description`, length must be less than or equal to `{self.CHAT_DESCRIPTION_MAX_LENGTH}`"
            )
    ...
    def start_payload(self, start_payload):
        ...
        if (
            ...
            raise ValueError(
                f"Invalid value for `start_payload`, length must be less than or equal to `{self.START_PAYLOAD_MAX_LENGTH}`"
            )
