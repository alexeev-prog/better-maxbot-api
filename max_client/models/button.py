...
class Button:
    ...
    discriminator_value_class_map = {
        "request_geo_location": "RequestGeoLocationButton",
        "chat": "ChatButton",
        "callback": "CallbackButton",
        "request_contact": "RequestContactButton",
        "link": "LinkButton",
    }
    ...
    def text(self, text):
        ...
        if text is not None and len(text) > self.MAX_TEXT_LENGTH:
            raise ValueError(
                f"Invalid value for `text`, length must be less than or equal to `{self.MAX_TEXT_LENGTH}`"
            )
