...
class UploadApi:
    ...
    def get_upload_url(self, type, **kwargs):
        r"""
        ...
        if kwargs.get("async_req"):
            return self.get_upload_url_with_http_info(type, **kwargs)
        return self.get_upload_url_with_http_info(type, **kwargs)

    def get_upload_url_with_http_info(self, type, **kwargs):
        r"""
        Get upload URL  # noqa: E501.
        ...
        if "type" not in local_var_params or local_var_params["type"] is None:
            raise ValueError(
                "Missing the required parameter `type` when calling `get_upload_url`"
            )
