...
class MarkupElement:
    ...
    def __init__(self, type=None, _from=None, length=None):
        """MarkupElement - a model defined in OpenAPI."""
        self._type = None
        self.__from = None
        self._length = None
            ...
            if sys.maxunicode != 0xFFFF:
                try:
                    ...
                    entity_text = entity_text[
                        markup._from * 2 : (markup._from + markup.length) * 2
                    ]
            ...
            if markup not in parsed_markups:
                ...
                if _from == 0:
                    if sys.maxunicode == 0xFFFF:
                        formated_text += (
                            OacUtils.escape(
                                message_text[last__from : markup._from - _from]
                            )
                            + insert
                        )
                    else:
                        formated_text += (
                            OacUtils.escape(
                                message_text[
                                    last__from * 2 : (markup._from - _from) * 2
                                ].decode("utf-16-le")
                            )
                            + insert
                        )
                    ...
                    formated_text += (
                        message_text[last__from : markup._from - _from] + insert
                    )
                else:
                    formated_text += (
                        message_text[
