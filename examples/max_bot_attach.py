...
from max_client import Configuration, Update, ApiClient, SubscriptionsApi, MessagesApi, BotsApi, ChatsApi, \
    ...
    GetSubscriptionsResult, Subscription, SimpleQueryResult, SubscriptionRequestBody, MessageChatCreatedUpdate, CallbackAnswer, UserWithPhoto, \
    User, TextFormat
from max_client.rest import ApiException, RESTResponse
from max_client.utl import OacUtils
from max_client.cmn import Utils, BotLogger
...
class MaxBot(object):
    ...
    def main_menu_buttons(self):
        ...
        buttons = [
            [LinkButton(_('API documentation for Max-bots'), 'https://dev.max.ru/docs')],
            [LinkButton(_('JSON Diagram API Max Bots'), 'https://github.com/max-messenger/max-bot-api-schema')],
    ...
    def check_threads(self):
        self.lgz.info('%s of %s threads are used.' % (len(MaxBot.threads), MaxBot.work_threads_max_count()))
        while len(MaxBot.threads) >= MaxBot.work_threads_max_count():
            err = 'Threads pool is full. The maximum number (%s) is used. Awaiting release.' % MaxBot.work_threads_max_count()
            self.lgz.debug(err)
            ...
            self.lgz.info('After trying to release: %s out of %s are used.' % (len(MaxBot.threads), MaxBot.work_threads_max_count()))
    ...
    def handle_request_body(self, request_body):
        ...
        if len(MaxBot.threads) < MaxBot.work_threads_max_count():
            ...
            err = 'Threads pool is full. The maximum number (%s) is used.' % MaxBot.work_threads_max_count()
            self.lgz.debug(err)
    ...
    def handle_message_edited_update(self, update):
        ...
        if update and update.chat_id:
            chat = self.chats.get_chat(update.chat_id)        
            return self.msg.send_message(NewMessageBody("отредактировал сообщение"), chat_id=chat.chat_id)
        #pass
    ...
    def subscribe(self, url_list, adding=False):
        ...
        for url in url_list:
            ...
            if isinstance(res, SimpleQueryResult) and not res.success:
                ...
                raise MaxBotException('Something went wrong when subscribing the WebHook %s' % wh_info)
